...
> 深度理解 → `.ai/core/principles.md`。冲突时右优先。
...
```
PRD 到达 → ⓪ PRD→Spec（单PRD提取 / 双PRD交叉对比）→ ① 画 Demo → ② 接口合并（可多轮）→ ③ 改造适配 → ④ 接口对接 → ⑤ 迭代修复
                             ↻ 分批到位                                        ↕
                                                                         自测 / 测试 / 上线
...
| 阶段 | 名称     | 触发信号                                   | 做什么（含独有约束）                                                                                                                                                         | ⚠️ 进入后必须 读取                | 🔒 输出锁（仅允许写这些路径）                                               | ✅ 必须产出                                                              |
| ---- | -------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| ⓪    | PRD→Spec | PRD + "拆前端范围/生成spec/蓝图/blueprint" | 单PRD→提取前端范围；双PRD→交叉对比（🔴🟡🟢分级，权威：类型/路径→后端，功能/交互→产品）。生成 spec.md + progress.md。**不写代码**                                             | `modes/prd-to-spec.md`            | `.ai/specs/{feature}/`                                                      | spec.md + progress.md                                                    |
| ①    | 画 Demo  | PRD/需求 + "画页面/出骨架/demo"            | 读模板+组件文档+错题集 → 生成占位API+骨架页面。占位URL: `/api/TODO/{module}`。单页面不生成spec；每个Task=一个页面                                                            | —（见阶段表）                     | `src/api/{module}/` + `src/pages/{module}/` + `specs/{feature}/`            | 占位API + 页面文件；多页面时加 spec.md + progress.md                     |
| ②    | 接口合并 | 提供 Swagger/接口文档                      | **条件分支**：有PRD→完整合并+feature-spec（🔴🟡🟢差异分级，权威：类型/路径→Swagger，功能/交互→PRD）；无PRD→仅生成API层。分批到达时追加合并                                   | `modes/api-merge.md`              | 分支A: `src/api/{module}/` + `specs/{feature}/`；分支B: `src/api/{module}/` | 分支A: **feature-spec.md（阻断性——未产出则阶段未完成）**；分支B: API代码 |
| ③    | 改造适配 | feature-spec就绪 + "改造/对齐规格"         | 读feature-spec → 生成变更清单 → 用户确认 → 执行。**仅修改已有文件，不新建**（新页面回阶段①）。改造是「对齐」不是「重写」，保留自定义逻辑。feature-spec中标`[待补充]`的不处理 | —（见阶段表） + 已有 feature-spec | `src/api/{module}/` + `src/pages/{module}/`（仅修改，禁止新建文件）         | 变更清单 → 用户确认 → 执行修改                                           |
| ④    | 接口对接 | 真实接口就绪 + "对接/联调/替换mock"        | 读已有代码 → **先对比再改**（不是重新生成）→ 替换占位URL，删除TODO。新增字段是否加到页面需确认。保留用户自定义逻辑                                                           | —（见阶段表）                     | `src/api/{module}/` + 用户确认的页面文件                                    | diff对比 → 确认 → 替换                                                   |
| ⑤    | 迭代修复 | "改一下/加字段/修复/调整"                  | 最小范围修改。≤20行(改)/≤50行(调整)/>50行暂停。已有代码风格优先，不做规范化重构。verify只修本次变更引入的新错误。禁止顺手重构。详见 `modes/incremental.md`                   | `modes/incremental.md`            | 仅用户指定的目标文件及其直接关联的类型文件                                  | 最小范围改动                                                             |
...
| 拿到 PRD + "拆前端范围/生成spec/蓝图/blueprint" | ⓪ PRD→Spec（自动检测单/双PRD走对应分支）     |
...
新页面模式（可编辑表格等）→ `.ai/templates/` 新增模板（可选）+ 第一节阶段表增行。仅当新模式有不可内联的独有规则表时才新增 mode 文件。
