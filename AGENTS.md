...
| 阶段 | 名称     | 触发信号                                | 做什么                                                       | ⚠️ 进入后必须 Read                         | 🔒 输出锁（仅允许写这些路径）                                               | ✅ 必须产出                                                              |
| ---- | -------- | --------------------------------------- | ------------------------------------------------------------ | ------------------------------------------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
...
> ⛔ **阻断性要求**：生成包含 SSearchTable / SForm / SButton / SDetail 的代码前，**必须执行** `Read .ai/sdesign/components/{组件名}.md`。未执行此步骤的代码视为无效，必须回滚重做。
...
> 验证阶段仅用于检查修复，禁止创建新文件。生成页面代码前必须 Read `.ai/pitfalls/index.md` 对照错题集。

---

## 四、项目结构

> 新建模块时 Read `.ai/core/architecture.md` 确认目录结构。
