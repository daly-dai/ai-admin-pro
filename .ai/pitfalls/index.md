...
| ---- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | --------------------------------------- |
| P001 | 列表页 + Modal/Drawer | **禁止在列表页管理弹层 open 状态**。必须创建容器组件（如 `{Entity}FormModal`），内部管理 open/mode/editId，通过 `useImperativeHandle` 暴露 `open()` 方法，列表页通过 ref 调用                 | 提取为子组件，用 ref 调用         | [详情](./modal-drawer-encapsulation.md) |
