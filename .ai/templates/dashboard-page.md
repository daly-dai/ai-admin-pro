...
| 组件                        | 来源            | 用途                          |
| --------------------------- | --------------- | ----------------------------- |
| echarts + echarts-for-react | npm             | 图表渲染                      |
| antd Row/Col                | antd            | 响应式网格                    |
| antd Card                   | antd            | 图表容器                      |
...
const DashboardPage: React.FC = () => {
  ...
  return (
    <Spin spinning={loading}>
      ...
      />
      <Row gutter={[16, 16]}>
        <Col span={6}>{stats && <StatCard data={stats.summary} />}</Col>
        <Col span={12}>{stats && <TrendChart data={trend} />}</Col>
        <Col span={6}>{stats && <PiePanel data={stats.distribution} />}</Col>
      </Row>
    </Spin>
  );
};
...
const TrendChart: React.FC<TrendChartProps> = ({ data, title }) => {
  ...
  if (!data.x.length) return <Empty description="暂无趋势数据" />;
