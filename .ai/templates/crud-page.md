...
| 模式       | 适用场景              | 实现方式                                                                  |
| ---------- | --------------------- | ------------------------------------------------------------------------- |
| **Modal**  | 字段 <= 8，无复杂联动 | `createModal`（`src/components/ModalContainer`）+ antd Modal + SForm      |
| **独立页** | 字段 > 8，含复杂控件  | `create.tsx` / `edit.tsx`，路由跳转                                       |
| **Drawer** | 仅查看详情            | `createDrawer`（`src/components/DrawerContainer`）+ antd Drawer + SDetail |
