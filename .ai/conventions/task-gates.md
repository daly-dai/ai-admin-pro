...
### page-dashboard — 大屏/仪表盘页面

**输出锁**:

- `src/pages/{module}/index.tsx` (新建/修改)
- `src/pages/{module}/components/StatCard.tsx` (新建)
- `src/pages/{module}/components/TrendChart.tsx` (新建)
- `src/pages/{module}/components/PiePanel.tsx` (新建)
- `src/pages/{module}/components/FilterBar.tsx` (新建)
- `src/pages/{module}/store.ts` (新建)

**验收闸门**:

- [ ] 使用 antd Row/Col 或 CSS Grid 响应式布局
- [ ] ECharts 实例在容器 resize 时正确重绘
- [ ] 轮询定时器在组件卸载时清除（ahooks useInterval 默认行为）
- [ ] 数据为空时显示 Empty 或 SNoData 占位
- [ ] filter 变化时重新请求，旧数据不残留
- [ ] 接口报错时图表区显示错误状态而非空白
- [ ] Store 类型完整（filter + data + loading）
- [ ] 全屏模式下无滚动条
- [ ] 全局闸门 G1-G12 全部通过
- [ ] `pnpm verify` 0 error（仅本Task输出锁内文件）

### component-chart — 图表组件

**输出锁**:

- `src/pages/{module}/components/{ChartName}.tsx` (新建/修改)

**验收闸门**:

- [ ] ECharts option 类型完整
- [ ] 使用 useSize 监听容器尺寸，触发 echartsInstance.resize()
- [ ] 数据为空时显示 Empty 占位
- [ ] Props 类型完整（data + config）
- [ ] 全局闸门 G1-G12 全部通过
- [ ] `pnpm verify` 0 error（仅本Task输出锁内文件）

### component-widget — 大屏小组件

**输出锁**:

- `src/pages/{module}/components/{WidgetName}.tsx` (新建/修改)

**验收闸门**:

- [ ] 数值格式化正确（toLocaleString / 单位 / 精度）
- [ ] 趋势箭头方向正确（正值↑ 负值↓）
- [ ] loading 态 + 空数据态
- [ ] Props 类型完整
- [ ] 全局闸门 G1-G12 全部通过
...
### data-transform — 数据转换层

**输出锁**:

- `src/pages/{module}/utils.ts` (新建/修改，或内联在组件中)

**验收闸门**:

- [ ] 函数签名类型完整（输入类型 + 返回类型）
- [ ] 边界值处理（null / undefined / 空数组）
- [ ] 全局闸门 G1-G12 全部通过
- [ ] `pnpm verify` 0 error（仅本Task输出锁内文件）
...
- `src/pages/{module}/index.tsx` (新建/修改)
- `src/pages/{module}/components/*.tsx` (按需)

**验收闸门**:

- [ ] 使用 Steps / Timeline 展示流程节点
- [ ] 每个步骤的审批/操作交互完整
- [ ] 状态流转与后端SDD一致
- [ ] 全局闸门 G1-G12 全部通过
- [ ] `pnpm verify` 0 error（仅本Task输出锁内文件）

### page-landing — 落地/门户页面
...
- [ ] 使用 antd Card / List 布局
- [ ] 导航入口链接正确
- [ ] 全局闸门 G1-G12 全部通过
- [ ] `pnpm verify` 0 error（仅本Task输出锁内文件）

### page-custom — 自定义页面（兜底）
