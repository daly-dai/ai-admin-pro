...
一个纠正**只沉淀到一个层**：按 L1→L2→L3→L4 优先级匹配到第一个合适的层即停止，不再继续往下。ESLint 的 `message` 字段同时承担引导正确写法的职责，无需额外层重复说明。
...
> **互斥原则**：每个纠正只命中一个层。匹配到第一个合适的层即停止，不再继续往下判断。
...
1. 晋升：该条目已被更高层覆盖 → 删除当前层条目
   （例：某个 L2 清单项后来有了对应的 ESLint 规则 → 从 L2 删除）
