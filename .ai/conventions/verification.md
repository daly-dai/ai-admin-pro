...
生成页面代码前读取 `.ai/pitfalls/index.md`，按「适用场景」匹配当前页面类型，逐条排查。
...
自修复：有错误 → 按优先级修复（tsc > eslint > prettier）→ 再次 verify → 最多 3 轮。连续 2 轮不减少 → 停止并报告当前状态。

**只处理当前 Task 输出锁范围内的错误。** 范围外报错 → 在报告中列出，不读取、不分析、不修复。
...
| Level 3 | 人工验证，标记未通过，回到对应 Task 修复 |
...
4. **三方库类型缺失**：检查 `@types/xxx`，无则用 `declare module` 补充声明文件
