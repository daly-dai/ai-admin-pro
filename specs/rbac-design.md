...
| 模块     | 状态        | 说明                                                      |
| -------- | ----------- | --------------------------------------------------------- |
| 用户管理 | ❌ 缺失     | 列表 + 新增/编辑 Modal + 详情 Drawer，`roleIds` 关联角色  |
| 角色管理 | ❌ 缺失     | 列表 + 独立表单页，`permissionIds` 关联权限，权限树硬编码 |
...
2. 角色选择下拉的数据源改为从 localStorage/Store 获取
...
1. **权限树动态化**：当前硬编码在 `pages/role/form.tsx` 中，改为从 `api/permission` 获取
2. 操作按钮增加权限控制
3. 删除角色时检查是否有用户关联此角色
...
| ------------- | --------------------------------------------------------------- |
| 角色列表      | SProTable + 搜索 + 分页                                         |
| 新增/编辑角色 | 独立表单页 + SForm + 权限树分配（从 `api/permission` 动态获取） |
| 删除角色      | Modal.confirm，检查是否有用户关联                               |
| 启用/停用     | Switch                                                          |
| 权限控制      | 操作按钮使用 `hasPermission` 包裹                               |
