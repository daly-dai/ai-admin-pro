...
### 🛑 防循环规则（优先级高于 §2 步骤表，每步执行前必检）

> ⛔ **当 §2 步骤表要求"读取文件 X"时，必须先对照此表判断是否真的需要读。**

| #   | 规则                                                                     | 违反后果 |
| --- | ------------------------------------------------------------------------ | -------- |
| R1  | **不重读刚写的文件**：最近 3 轮内写入/编辑过的文件，内容已知，禁止再读取 | 浪费轮次 |
| R2  | **不重读刚读的文件**：最近 5 轮内已读取过的文件，禁止再读取              | 车轱辘话 |
| R3  | **同一文件最多编辑 3 次**：编辑 ≥3 次仍有错误 → **停止，报告用户**       | 越改越错 |
| R4  | **写完直接验证**：文件写入完成后直接 `pnpm verify`，不要先"读一遍检查"   | 自欺欺人 |
| R5  | **3 轮无进展即停止**：连续 3 轮同类错误不减少 → **强制停止，报告状态**   | 死循环   |

> ⛔ **R1-R5 优先级最高**。即使 §2 步骤表要求读文件，只要 R1/R2 判定已读过/写过，就跳过读取。
